...
describe("score.svg() emitter (single-system)", () => {
  const score = buildScore(makeChant(KYRIE_GABC));

  test("score.svg() renders a self-contained staff with notes and lyrics", () => {
    const svg = score.svg();
  ...
  test("score.svg() sizes glyphs to the SMuFL standard (staff space = upm/4)", () => {
    // staffHeight 48 → staffInterval 8 → staff space 16 px → scale 16/250.
    const svg = buildScore(makeChant("(c4) a(g) (::)")).svg({ staffHeight: 48 });
  ...
  test("score.svg() renders the clef from the score and moves it by line", () => {
    // c4 → do clef on the top line (position 7 → y 40 at staffHeight 48);
    // c3 → line 3 (position 5 → y 56). Same letters, same slots, clef moves.
    const c4 = buildScore(makeChant("(c4) a(g) (::)")).svg({ staffHeight: 48 });
    const c3 = buildScore(makeChant("(c3) a(g) (::)", "1")).svg({ staffHeight: 48 });
    ...
    const f3 = buildScore(makeChant("(f3) a(g) (::)")).svg({ staffHeight: 48 });
  ...
  test("score.svg() renders a clivis as two abutting notes with a left stem", () => {
    const svg = buildScore(makeChant("(c4) a(hg) (::)")).svg();
  ...
  test("score.svg() renders a torculus with junction stems and a porrectus swash", () => {
    const torc = buildScore(makeChant("(c4) a(ghg) (::)")).svg();
    assert.equal((torc.match(/class="note"/g) || []).length, 3);
    assert.equal((torc.match(/class="stem"/g) || []).length, 2, "stems at both junctions");
    const porr = buildScore(makeChant("(c4) a(hgh) (::)")).svg();
    assert.equal((porr.match(/swash/g) || []).length, 1, "porrectus keeps the diagonal swash");
  });

  test("score.svg() draws explicit accidentals and ledger lines", () => {
    // gx = explicit flat; letter a sits below the staff → ledger line at −1.
    const svg = buildScore(makeChant("(c4) a(gx)b(a) (::)")).svg();
  ...
  test("score.svg() renders mora, episema, and ictus signs", () => {
    const svg = buildScore(makeChant("(c4) a(g.)b(g_)c(g') (::)")).svg();
  ...
  test("score.svg() hyphenates syllables within a word", () => {
    // Ky- ri- e | e- ia: hyphens are appended to intra-word syllables.
    const svg = buildScore(makeChant("(c4) Ky(g)ri(h)e(g) e(f)ia(g) (::)")).svg();
    assert.equal((svg.match(/-<\/text>/g) || []).length, 3);
    assert.ok(svg.includes(">Ky-<"), "hyphen rides the syllable text");
  });

  test("score.svg() uses the quilisma glyph when a note is a quilisma", () => {
    const plain = buildScore(makeChant("(c4) a(fh) (::)")).svg();
    const quil = buildScore(makeChant("(c4) a(fwh) (::)")).svg();
