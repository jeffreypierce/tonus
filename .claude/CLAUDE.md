...
**`docs/` is the published site.** GitHub Pages serves it, so it holds the
playground (`index.html`, `app.js`, `styles.css`, `diagrams/`, `fonts/`) with
the API markdown beside it in **`docs/api/`** — framed into the site rather
than kept apart from it. `docs/dist/` is the built library the page imports;
it is generated by `npm run site` and gitignored.

`npm run serve` runs it at <http://localhost:8000>. It must be served over
http: the page is ES modules, and a browser treats every `file://` URL as its
own origin, so opening `docs/index.html` directly fails every import with a
CORS error that looks like a bug in the site and is not one.
...
- Green tests **and** clean `tsc` at every commit. (Current floor: 594 tests.)
