...
That boundary has teeth. Where the site draws something the library also names
— the Guidonian hand is the standing case, a figure of coordinates against the
gamut's fingers — the two tables must agree, and the test that holds them
together lives with the FIGURE, read against the vendored engine the browser
actually loads. Testing the installed package instead passes while the site
ships a stale vendor tree; that is exactly how the cc/dd swap survived.
...
- **`.claude/standards/DOCS-STANDARDS.md`** — the two-level documentation
  ladder (`docs/api/*.md` → code), the centralized bibliography, and the one-voice
...
- Green tests **and** clean `tsc` at every commit. (Current floor: 750 tests.)
